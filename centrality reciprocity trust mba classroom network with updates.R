...
library(igraph)
library(data.table)
setwd("C:/Users/demetrius/Dropbox/Social Network Analytics ISOM 673 Fall 2018")
#setwd("~/Dropbox")

# task 1 -- getting adjacency matrix data into igraph
data = fread("C:/Users/akabo/Downloads/social network analysis/anonymized_mba_classroom_network.csv", header = TRUE)
...
# then, insert a row of 0s for participants who did not fill out the survey, so they can still be represented in terms of ties received
...
plot.igraph(trust,vertex.label=NA,layout=layout.fruchterman.reingold, vertex.label.color="black",edge.color="blue",edge.width=E(trust)$weight,vertex.size = 12, edge.arrow.size=.3,edge.curved=FALSE)


# task 3 -- plot the distrust relationships according to their strength and direction, and also color nodes in terms of how distrusted they are

# distrusting data must be numeric. igraph ignores blanks, which is okay for trust graph because no isolates, but for distrusting need blanks --> 0s for isolates to appear on the plot
...
plot.igraph(distrust,vertex.label=NA,layout=layout.fruchterman.reingold, vertex.label.color="black",edge.color="black",edge.width=E(distrust)$weight,vertex.size = 12, edge.arrow.size=.3,edge.curved=FALSE)# igraph also gives the solution via degree(distrusting, mode = "in")

# solution 1 using rainbow, one way to label ordinally: start = 0 with end = 1/6 produces a gradient from red to yellow as distrustedness grows high
colrange = rainbow(length(unique(distrusted)), start = 0, end = 1/6) # colrange is like the colors object from the igraph example but produces a gradient instead
...
# solution 2 using colorRampPalette, another way to label ordinally -- here input "red" and "yellow" by name and assign color through raw vertex attribute
...
plot.igraph(distrust,vertex.label=NA,layout=layout.fruchterman.reingold, vertex.label.color="black",edge.color="black",edge.width=E(distrust)$weight,vertex.size = 12, edge.arrow.size=.3,edge.curved=FALSE)


# task 4 -- plot the advice network, coloring the node who the most individuals go to for advice in a different color
...
# 32 is missing from advice network -- can add manually to edge list as an isolate by adding as a self-loop and then simplifying to remove the loop
...
# task 5 -- how many of the trust and advice relationships are reciprocated? is this more or less than random chance? for this portion, treat the trust and distrust relationships as binary (i.e., not weighted)\
...
# can look at overlap of ties going in each direction, 1 from each source = 2. this gives a count of how many people sent a tie that was also received. to get the number of relationships, divide the number of people by two
sum(trust_matrix + t(trust_matrix) == 2)/2
...
# for reciprocation to be totally up to chance, reciprocation and not reciprocation are equally as likely--50% chance of occurring

# divide number of reciprocated relationships by total relationships. number of total relationships is equal to reciprocated plus non-reciprocated. 1 and 0 coming from each source = 1. gives a count of how many people sent a tie that was not sent back. to get the number of relationships, divide this number by two
(sum(trust_matrix + t(trust_matrix) == 2)/2) / (sum(trust_matrix + t(trust_matrix) == 2)/2 + sum(trust_matrix + t(trust_matrix) == 1)/2)

# can also use the matrix formula: number of non-reciprocated relationships is given by trace(network*t(network)) - trace(network*network)
(sum(diag(trust_matrix%*%trust_matrix))/2)/(sum(diag(trust_matrix%*%trust_matrix))/2 + sum(diag(trust_matrix%*%t(trust_matrix))) - sum(diag(trust_matrix%*%trust_matrix)))
...
(sum(diag(distrust_matrix%*%distrust_matrix))/2)/(sum(diag(distrust_matrix%*%distrust_matrix))/2 + sum(diag(distrust_matrix%*%t(distrust_matrix))) - sum(diag(distrust_matrix%*%distrust_matrix)))
...
(sum(diag(advice_matrix%*%advice_matrix))/2)/(sum(diag(advice_matrix%*%advice_matrix))/2 + sum(diag(advice_matrix%*%t(advice_matrix))) - sum(diag(advice_matrix%*%advice_matrix)))

# somewhat less reciprocation than by chance

# task 6 -- figure out how much overlap is there between the relationships in the trust network and the advice networks

# if relationships exist in both networks, will receive a "1" from each

# note that it's necessary to sort the advice matrix on the row and column names so the vertices line up with one another (igraph will do this automatically with the V(graph)$name parameter)
...
# plot the overlap between the trust and advice networks, indicating which relationships are trust only, advice only, or both


# manually manipulate weights to identify the presence of tie
advice_matrix[advice_matrix == 1] = 4
...
# since edges are in the same order as before, can just recycle the weights from earlier in the plotting parameters to preserve strength of trust relationship
plot.igraph(trusting_advice,vertex.label=NA,layout=layout.fruchterman.reingold, vertex.label.color="black",edge.color=ranges,edge.width=E(trust)$weight,vertex.size = 12, edge.arrow.size=.3,edge.curved=FALSE)
